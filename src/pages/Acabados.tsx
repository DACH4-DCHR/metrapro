...
function nextName() {
  const count = useProjectStore.getState().elements.filter((e) => e.module === "acabados").length;
...
export function AcabadosPage() {
  ...
  const [input, setInput] = useState<AcabadosInput>({
    ...
    incluirPinturaMuros: true,
    incluirPinturaCielorraso: false,
  });
  ...
  if (input.incluirPinturaMuros) {
    lines.push({
      partida: "Pintura látex 2 manos en muros interiores",
      unidad: "m²",
      cantidad: result.areaMurosNeta,
    });
  }
  if (input.incluirPinturaCielorraso) {
      ...
      inputsSummary: {
        ...
        Partidas:
          [
            ...
            input.incluirPinturaMuros && "pintura de muros",
            input.incluirPinturaCielorraso && "pintura de cielorraso",
          ]
            .filter(Boolean)
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Tarrajeo y pintura de muros y cielorraso, por ambiente (cuarto, baño, sala, etc.)"
          ...
          <div className="flex flex-col gap-6">
            ...
            <SectionCard title="Partidas a incluir" icon={<PaintRoller size={16} className="text-navy-700" />}>
              <div className="flex flex-col gap-3">
                ...
                />
                <CheckboxField
                  label="Pintura de muros interiores"
                  checked={input.incluirPinturaMuros}
                  onChange={(v) => update("incluirPinturaMuros", v)}
                />
                <CheckboxField
                  label="Pintura de cielorraso"
                  checked={input.incluirPinturaCielorraso}
                  onChange={(v) => update("incluirPinturaCielorraso", v)}
                />
              </div>
            </SectionCard>
          </div>
